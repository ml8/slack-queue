...
gcloud compute instances create-with-container slack-queue \
  --container-image gcr.io/${SQ_GCP_PROJECT}/slack-queue:latest \
  --machine-type f1-micro \
  --tags app \
  --address ${SQ_GCP_IP} \
  --disk name=slack-data-disk,mode=rw \
  --container-mount-disk mount-path="/disks/data-disk",name=slack-data-disk,mode=rw \
